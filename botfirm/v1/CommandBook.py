...
class CommandBook(BotInterface_v1):
    botname = "command_book"
    agent_name = "command_book"
    display_name = "しょ！ (書籍 調査班)"
    useful_when = "Search and find book information"
    description = """Book (しょせき＝＞しょ) 調査 BOT
書籍名、著者名、キーワードなど、書籍に関する情報であると仮定してキーワード調査を行う
...
"""
    ...
    async def bot_response(self, message: Message_v1, history: ChatHistory) -> list[Message_v1]:
        ...
        book_hints = [l for l in LineParse(content, skip_empty=True)]

        # build markdown myself
        nowtime = datetime.datetime.now().isoformat()
        response = f"# book check @ {nowtime}\n\n"
